...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
hotels%>%filter(children>=1,hotel=='Resort hotel')%>%select(hotel,children)
...
hotels%>%mutate(little_ones=children+babies)%>%select(hotel,little_ones)
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
hotels%>%summarise(mean=mean(adr))
...
```{r, eval=TRUE,echo=TRUE}
# Enter code here
hotels%>%group_by(hotel)%>%summarise(mean=mean(adr))
```
...
slice(1:5)%>%arrange(lead_time)

hotel%>%select(hotel, lead_time)%>%slice(1:5)%>%arrange(lead_time)
```
