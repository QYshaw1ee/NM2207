...
**Solution:** CSV stands for comma separated values. It contains  plain data by delimiting data entries with commas. They are popular for storing data because they can be open as text editors, spreadsheet programmes like Excel or in R studio. Therefore, csv's flexibility makes it popular to store data for multiple uses. 
<br>
<br>
...
music <- read.csv(playlist_data.csv) 
...
knitr::include_graphics("challege.png")
```
<br>

**Question 1.6:** What does the `skip` argument in the read_csv() function do?

**Solution:** The 'skip' argument in read_csv lets me skip certain rows from playlist_data.csv when I import data from the csv file playlist_data.csv. 
<br>
<br>
...
ggplot(data=music)+aes(x=Age,y=Rating)
...
ggplot(data=music,mapping=aes(x=Age,y=Rating))+labs(x="Age",y="Rating")
...
ggplot(data=music,mapping=aes(x=Age,y=Rating)) +
geom_point(shape="\u2620",size=10) # <-- Hint: Use ? to learn more about geom_point and use appropriate values for shape
